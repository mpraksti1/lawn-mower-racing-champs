...
const StyledButton = styled.button`
  ...
  border-radius: 2px;
  margin: ${props => {
    if (props.spaceAround) return `${props.theme.baseSizeUnit}px 0`;
    if (props.spaceAbove) return `${props.theme.baseSizeUnit}px 0 0 0`;
    if (props.spaceBelow) return `0 0 ${props.theme.baseSizeUnit}px 0`;
    return 0;
  }};

  /* For medium variant */
  ${ ({medium, theme}) => medium && css`
    background: ${() => theme.fontSizes.md};
  `}

  /* For large variant */
  ${ ({large, theme}) => large && css`
